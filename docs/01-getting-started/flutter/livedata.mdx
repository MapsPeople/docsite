...
Enabling Live Data through [`MapsIndoorsWidget`](example.com) is as simple as [calling `mapsindoorsWidget.enableLiveData()`](example.com) with a [Domain Type](example.com).
...
In the context of your view controller showing a map, add the call after your `MapsIndoorsWidget` object is ready. We can add a button to our map, that when pressed calls ``enableLiveData``
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: TextButton(
        onPressed: enableLiveData,
        style: const ButtonStyle(backgroundColor: MaterialStatePropertyAll(Colors.white)), 
        child: const Text("Enable Livedata",),
      ),
...
<img src="/img/getting-started/flutter_livedata.gif" alt="An animation showing the desired behaviour of this tutorial" width="350"/>
