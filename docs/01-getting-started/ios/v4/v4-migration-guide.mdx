...
To create MPLocationUpdate objects, you would simply call the `init`:
...
The concept is still very similar to that of on v3. In that you create custom Location Sources except to create `MPLocationUpdate`.
 ...
 Task {
        ...
        ***/
        MPMapsIndoors.shared.register([
            PeopleLocationSource(type: "Robot")
            ])
```