...
<Tabs>
<TabItem value="v4" label="iOS SDK V4">
...
- DisplayRules with now support unlimited zoom levels. The feature will be available in the MapsIndoors CMS soon.
- `MPSelectionBehavior` now has the `zoomToFit` property.

### Fixed

- Setting the `icon` property of a DisplayRule now works as expected.
- Fixed an issue with map items not showing immediately, only when map was moved slightly.
- Fixed an issue that would cause MapsIndoors tiles not to be shown when navigating between venues.
