...
<Tabs groupId="data-booking-platform">
...
In `numberOfSections(in tableView:)` add:
...
```swift
let bookingService = MPBookingService.sharedInstance()
let bookableQuery = MPBookableQuery()

bookableQuery.startTime = Date()
...
```swift
private let bookBtn = UIBarButtonItem(title: "Create Booking", style: .plain, target: self, action: #selector(book))
private let cancelBtn = UIBarButtonItem(title: "Cancel Booking", style: .plain, target: self, action: #selector(cancel))
```
