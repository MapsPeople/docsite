...
```swift
MPMapsIndoors.shared.bookingService.bookingsUsing(query: MPBookingsQuery)
```
...
To determine whether or not a Location is bookable can be looked up using the `MPMapsIndoors.shared.bookingService.bookingsUsing(query: MPBookingsQuery)` method. Below is an example of querying for bookable Locations:
