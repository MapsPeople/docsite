...
1. Navigate to `Solution Details` -> `Solution Settings` -> `2D Model`. Here you should find a preview image, and underneath, a button that says `Remove`. This removes any default 2D model on the Main Display Rule.
