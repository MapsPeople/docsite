...
1. **Visibility** - Controls whether the 2D model is visible on the map or not.
1. **Zoom from** - Controls at which zoom level the 2D model STARTS being visible.
1. **Zoom to** - Controls at which zoom level the 2D model STOPS being visible (usually set to the highest setting, so they are visible at maximum zoom).
1. **2D Model** - Open the Media Library, where you can choose which 2D model to display.
1. **Width x Height** - Controls the width and height of the 2D model, measured in meters. These values cannot be changed individually, the 2D model must maintain it's original aspect ratio.
1. **Bearing (rotation)** - Controls the rotation of the 2D model.
...
1. Use the `Visibility` toggle on the Display Rules panel. This will leave the model displayed in the CMS, but your cusotmers will not see it.

1. Navigate to `Solution Details` -> `Solution Settings` -> `2D Model`. Here you should find a preview image, and underneath, a button that says `Remove`. This removes any default 2D model on the Main Display Rule, and the Display Rule hierachy will then ensure nothing is displayed if you untoggle `2D Model` on the Location in question.
...
1. **Visibility** - Controls whether the 3D walls are visible on the map or not.
1. **Zoom from** - Controls at which zoom level the walls START being visible.
1. **Zoom to** - Controls at which zoom level the walls STOP being visible (usually set to the highest setting, so they are visible at maximum zoom).
1. **Walls color** - Controls the color of the 3D walls.
...
1. **Visibility** - Controls whether the room is extruded on the map or not.
1. **Zoom from** - Controls at which zoom level the extrusion STARTS being visible.
1. **Zoom to** - Controls at which zoom level the extrusion STOPS being visible (usually set to the highest setting, so they are visible at maximum zoom).
1. **Walls color** - Controls the color of the room extrusion.
1. **Wall height** - Controls the height of the room extrusion, measured in meters.
