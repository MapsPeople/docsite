...
The goal is to show the name of the catagores, image of the catagories and then the location names. By following this guide, you can create a working app based on your `AppConfig`. This
...
Handle row selection using the UITableViewDelegate method. Keep in mind that the `subMenuViewController` will be crated in part two. To make the app run without the `subMenuViewController`, you can outcomment the `subMenuViewController` part, for now.
```swift 
...
This class allows you to display a menu with categories, using the MapsIndoors SDK. Next step is to use `AppConfig` to filter `MPLocations` on a solution. This can be seen in 
part 2. 
...
Create a method inside the `SubMenuViewController`. This method will load MapsIndoors and fetch locations using a filter of witch the catagory key is used.
