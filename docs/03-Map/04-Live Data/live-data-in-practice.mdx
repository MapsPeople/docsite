...
```swift
 Task {
        let q = MPQuery()
        let f = MPFilter()
        let locations = await MPMapsIndoors.shared.locationsWith(query: q, filter: f)
...
In this tutorial you will learn to work with Live Data / Real Time Data in MapsIndoors. It is recommended that you read the [Live Data Introduction](/live-data-intro/) before continueing.
