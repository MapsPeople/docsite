...
Obviously it is up to your application to present some instructions to the end user, but here a suggestion. Add a method called `updateViews` that will fire whenever our models change. Initialise an array of textual instructions and check for existence of a current leg.
...
Create a `setupMap` method that sets up the Google map and MapsIndoors Map Control object. To configure a `mapConfig` see [Getting Started](/getting-started/ios/v4/display-a-map/)
...
fileprivate func setupMap() async {
    ...
    MapEngine.selectedMapProvider?.setCamera(coordinates: location, zoom: 20)
