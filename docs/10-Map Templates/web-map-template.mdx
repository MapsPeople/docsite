...
* You can use both the `SSH` or `HTPPS` to clone the repository, depending on which one you are more familiar with.
```bash
...
6. `appUserRoles` -  Used like this `appUserRoles=visitor,staff,security` - the appUserRoles are case sensitive. **Note!** You need to provide a list of comma separated values, **without** any spaces between the comma and the value. This will further be converted into an array of appUserRoles.
...
`locationId` + `startZoomLevel` → the `locationId` has priority over the `startZoomLevel`
